...
// - doc:{id}:updates
// - doc:{id}:presence
function docUpdatesChannel(documentId) {
    return `doc:${documentId}:updates`;
}
function docPresenceChannel(documentId) {
    return `doc:${documentId}:presence`;
}
const REDIS_APPLY_ORIGIN = { kind: "redis-apply" };
const REDIS_INSTANCE_ID = crypto.randomUUID();
const bridgedDocs = new Map();
function extractDocIdFromReq(req) {
...
function getTokenFromReq(req) {
    ...
    const bearer = typeof authHeader === "string" ? authHeader.replace(/^Bearer\s+/i, "").trim() : undefined;
    return tokenFromQuery ?? bearer;
}
async function userCanAccessDocument(documentId, userId) {
...
export async function setupYjsWebsocket(httpServer) {
    ...
    async function bridgeDocToRedis(documentId) {
        if (bridgedDocs.get(documentId))
            return;
    ...
    async function handleRedisMessage(_pattern, channel, message) {
        ...
        if (!parsed)
            return;
        if (parsed.instanceId === REDIS_INSTANCE_ID)
            return;
        if (kind === "updates") {
            if (!parsed.update)
                return;
